...
export function createCanvasMCPServer(config) {
    return new CanvasMCPServer(config);
}
export function createHandlers(canvasAPI) {
    const createResourceMetadata = (course) => ({
    ...
    return {
        ...
        async handleSearchResources(request) {
            const courses = await canvasAPI.listCourses(['term', 'total_students']);
    ...
    };
    {
        ...
        void {
            this: .server.setRequestHandler(ListResourcesRequestSchema, this.handlers.handleListResources),
            this: .server.setRequestHandler(ReadResourceRequestSchema, this.handlers.handleReadResource),
            this: .server.setRequestHandler(SearchResourcesRequestSchema, this.handlers.handleSearchResources),
            this: .server.setRequestHandler(WatchResourcesRequestSchema, this.handlers.handleWatchResources)
        };
        async;
        startWatchPolling();
        Promise < void  > {
